...
## Strength gate
`node scripts/match-arena.mjs --games 64 --skill N --movetime 100 --fairytime 100` (drop `--fairytime` and fairy gets 4×). **Games run concurrently** (`--concurrency N`, default 6 → ~12 busy cores): measured **5.4–5.9× vs serial**, and each game clears both TTs so a block is independent of slot assignment. Full ladder: `bash scripts/ladder.sh 64`.
...
_Generated from `results/blocks.jsonl` (10 live blocks). Do not hand-edit — run `node scripts/results.mjs --write-agents`._
